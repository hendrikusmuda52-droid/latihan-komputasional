...
export async function POST(req: NextRequest) {
  try {
    ...
    const { title, content, targetKelas, category, isActive, cpId, tpId } = body as {
      title?: string
      content?: string
      targetKelas?: string
      category?: string
    ...
    // Ensures pasted text preserves paragraph breaks (Bug #2 fix)
    ...
    // cpId/tpId are optional — if empty/null/undefined, pass null so the FK
    // constraint doesn't block the insert when no CP/TP has been selected.
    try {
