...
export function TypingStage() {
  ...
  // We call handleFinish() to submit the final result to /api/result,
  // so the student's work is saved even though they're being redirected.
  useEffect(() => {
    const handleForceStopExpired = () => {
      try {
        handleFinish()
      } catch (err) {
        console.error('[typing-stage] force-stop finish failed:', err)
      }
    }
  ...
  const handleFinish = () => {
    ...
    // ── FIX: Check taskType from localStorage ──
    // If taskType is 'typing_only', skip quiz entirely — go straight to submit result + completed.
    // If taskType is 'typing_quiz' (or anything else), proceed to typing-finished → quiz.
    const taskType = typeof window !== 'undefined' ? localStorage.getItem('currentAssignmentTaskType') || 'typing_quiz' : 'typing_quiz'
    const isTypingOnly = taskType === 'typing_only'
    ...
    if (student?.id) {
      ...
      fetch('/api/student/progress', {
        ...
        body: JSON.stringify({
          studentId: student.id,
          currentStage: isTypingOnly ? 'completed' : 'typing-finished',
          typedText: typed,
          charCount: typed.length,
          correctChars,
          ...
          quizAnswers: '{}',
          isCompleted: isTypingOnly, // mark complete if typing-only
        }),
      }).catch(err => console.error('Failed to save progress:', err))
    }

    if (isTypingOnly) {
      // ── TYPING ONLY: Submit result directly, skip quiz ──
      const assignmentId = typeof window !== 'undefined' ? localStorage.getItem('currentAssignmentId') : null
      const totalScore = result.typingScore // 100% from typing
      ...
      fetch('/api/result', {
        ...
        body: JSON.stringify({
          ...
          typingDuration: result.typingDuration,
      ...
      }).catch(err => console.error('Failed to submit typing-only result:', err))

      // Go directly to completed — no quiz stage
      setStage('completed')
    } else {
      // ── NORMAL FLOW: typing → typing-finished → quiz ──
