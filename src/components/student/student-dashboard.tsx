...
export function StudentDashboard({ student, onLogout }: { student: StudentInfo; onLogout: () => void }) {
  ...
  if (loading) return (<div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-emerald-50 via-white to-teal-50"><RefreshCw className="w-12 h-12 animate-spin text-emerald-500 mx-auto mb-4" /><p className="text-slate-500">Memuat dashboard...</p></div>)
      ...
      <header className="sticky top-0 z-40 bg-gradient-to-r from-emerald-600 via-teal-600 to-emerald-700 shadow-lg">
        <div className="container max-w-5xl mx-auto px-4 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div><h1 className="text-lg font-bold text-white">SAKOLA</h1><p className="text-xs text-emerald-100">{student.namaLengkap} • {student.kelas}</p></div>
          </div>
          <div className="flex items-center gap-2">
            ...
            <Button variant="outline" size="sm" onClick={fetchData} disabled={loading} className="bg-white/10 border-white/20 text-white hover:bg-white/20"><RefreshCw className={`w-4 h-4 ${loading ? 'animate-spin' : ''}`} /></Button>
      ...
      <main className="container max-w-5xl mx-auto px-4 py-6 space-y-6">
        <div className="bg-gradient-to-r from-emerald-500 to-teal-600 rounded-2xl p-6 text-white shadow-xl relative overflow-hidden">
          ...
          <div className="relative z-10">
            ...
            <p className="text-emerald-100 text-sm">NISN: {student.nisn} • {student.sekolah}</p>
        ...
        {data && data.results.length > 0 && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {[
              ...
              <Card key={i} className={`${s.bg} border-0 shadow-md hover:shadow-lg transition-shadow`}>
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-3 text-xs text-slate-400 mt-3">
                          ...
                          {a.questionCount > 0 && <span className="flex items-center gap-1"><Brain className="w-3 h-3" />{a.questionCount} soal</span>}
                        </div>
                      </div>
        ...
        {activeTab === 'materi' && (
          <div className="space-y-3">
            {materials.length === 0 ? (
              <Card className="border-0 shadow-md"><CardContent className="py-16 text-center"><div className="w-16 h-16 rounded-full bg-purple-50 flex items-center justify-center mx-auto mb-4"><BookOpen className="w-8 h-8 text-purple-400" /></div><p className="font-semibold text-slate-700">Belum ada materi</p></CardContent></Card>
            ) : (
        ...
        {activeTab === 'nilai' && (
          <div className="space-y-3">
            {!data?.results.length ? (
              <Card className="border-0 shadow-md"><CardContent className="py-16 text-center"><div className="w-16 h-16 rounded-full bg-amber-50 flex items-center justify-center mx-auto mb-4"><Award className="w-8 h-8 text-amber-400" /></div><p className="font-semibold text-slate-700">Belum ada nilai dirilis</p></CardContent></Card>
            ) : (
