...
import {
  ...
  Wallet, KeyRound, Pencil, AlertCircle,
} from 'lucide-react'
...
export function AdminManager() {
  ...
  const { data: usersData, loading, error, refetch, retryCount } = useResilientFetch<{
    success: boolean
    teachers: UserRow[]
  }>('/api/teacher/manage', { deps: [] })

  const users = usersData?.teachers ?? []
  const fetchUsers = useCallback(() => { refetch() }, [refetch])
        ...
        <CardContent className="pt-0">
          ...
          users.length === 0 ? (
            <div className="py-20 text-center text-slate-400">
              ...
              <p className="text-xs mt-1">Klik "Tambah User" untuk membuat akun guru pertama</p>
            </div>
          ) : (
