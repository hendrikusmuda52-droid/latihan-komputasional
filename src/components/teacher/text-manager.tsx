...
import { Plus, Pencil, Trash2, CheckCircle2, Circle, RefreshCw, AlertCircle } from 'lucide-react'
...
export function TextManager() {
  ...
  // ── RESILIENT FETCH: auto-retry on 401/network error, preserves last data ──
  const url = filterGrade === 'ALL' ? '/api/typing-texts' : `/api/typing-texts?grade=${filterGrade}`
  const { data: textsData, loading, error, refetch, retryCount } = useResilientFetch<{
    success: boolean
    texts: TypingText[]
  }>(url, { deps: [filterGrade] })

  const texts = textsData?.texts ?? []

  // Stable refetch callback for use in event handlers
        ...
        <CardContent className="pt-0">
          ...
          {loading ? (
            <div className="py-20 text-center text-slate-400">
              ...
              <p className="text-sm">Memuat data teks bacaan...</p>
              {retryCount > 0 && (
          ...
          texts.length === 0 ? (
            <div className="py-20 text-center text-slate-400">
              ...
              <p className="text-xs mt-1">Klik &quot;Tambah Teks&quot; untuk membuat teks bacaan pertama</p>
            </div>
          ) : (
