...
# ── PERMANENTLY REMOVED: SQLite custom.db path and fallback ──
# SAKOLA uses Supabase PostgreSQL exclusively. No SQLite allowed.
# Any attempt to set DATABASE_URL to file: will cause FATAL exit.
...
if [ -f "./next-service-dist/server.js" ]; then
    ...
    # ── REQUIRE PostgreSQL — never fall back to SQLite ──
    if [ -z "$DATABASE_URL" ] || echo "$DATABASE_URL" | grep -q "^file:"; then
        echo "❌ FATAL: DATABASE_URL must be set to PostgreSQL (Supabase)."
        echo "   SQLite fallback has been permanently disabled."
        echo "   Set DATABASE_URL in environment variables or .env.local"
        exit 1
    fi
