...
# ── REMOVED: SQLite custom.db copy — SAKOLA uses Supabase PostgreSQL exclusively ──
# The old code below copied a local SQLite file and ran db:push on it.
# This caused .env to be silently overwritten with SQLite DATABASE_URL.
# SAKOLA now uses Supabase PostgreSQL via environment variables only.
# Do NOT re-enable this block.
echo "ℹ️  Database: Supabase PostgreSQL (configured via environment variables)"
echo "ℹ️  Skipping local SQLite copy (deprecated)"
